<template>
  <b-container fluid>
    <b-row>
      ...
      >
        <template #empty>
          <h6 class="p-5 text-center">No Notes available at the moment!</h6>
        </template>
  ...
  methods: {
    deleteItem(item) {
      this.$bvModal
        ...
        .then(async (value) => {
          if (value) {
            this.$bvToast.show("delete-note-toast");
            const reqData = {
            ...
            try {
              ...
              this.$bvToast.hide("delete-note-toast");
