<template>
  <b-container class="school-quizes p-5">
...
export default {
  ...
  computed: {
    ...
    allQuizes() {
      return this.$store.state.school.quizes;
    },
    user() {
  ...
  methods: {
    async fetchQuizes() {
      ...
      const reqData = { action: "getQuiz", school: this.user.school };
      try {
        const { data } = await this.axios.post("getQuiz", reqData);
        this.state.loadingQuizes = false;
        this.$store.dispatch("school/SET_SCHOOL_QUIZ", data);
      } catch (error) {
        this.state.loadingQuizes = false;
        console.log(error);
      }
    ...
    openModal() {
      this.$bvModal.show("school-add-quiz-modal");
    },
    hideModal() {
