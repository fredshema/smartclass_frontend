<template>
  <b-col cols="12" id="challenge">
    ...
    <b-collapse :id="collapseID">
      ...
      <b-row class="mb-4" no-gutters align-h="center" v-if="!allAnswers">
        <b-col cols="auto">
          <b-button variant="dark" class="w-100 px-3" @click.once="loadAnswers">
            <transition-group name="group-fade">
              <span key="loading" v-if="state.loadingAnswers">
                <AppLoading size="sm" />
              </span>
              <span key="text" v-else> Load Answers</span>
            </transition-group>
          </b-button>
        </b-col>
      </b-row>
      <b-row no-gutters class="mb-4" v-else>
        <b-col class="p-2">
          <b-button variant="dark" class="w-100" v-b-toggle:[studentCollapseID]>
            View collegues Answers
          </b-button>
        </b-col>
        <b-col class="p-2">
          <b-button variant="dark" class="w-100" v-b-toggle:[teacherCollapseID]>
            View Teacher answer
          </b-button>
        </b-col>
      </b-row>
      ...
      <!-- teacher collapse -->
      <b-collapse :id="teacherCollapseID" class="mt-4">
        <b-row no-gutters>
          <h6>Teacher's Answer:</h6>
        </b-row>
        <b-list-group>
          <b-list-group-item
            class="bg-light"
            v-for="(answer, i) in teacherAnswers"
            :key="i"
          >
            <h6>{{ answer.teacherAnswer.name }}</h6>
            <article v-html="answer.teacherAnswer.answer" />
          </b-list-group-item>
        </b-list-group>
      </b-collapse>

      <!-- student collapse -->
      <b-collapse :id="studentCollapseID" class="mt-4">
        <b-row no-gutters>
          <h6>Collegues Answers:</h6>
        </b-row>

        <b-list-group>
          <b-list-group-item v-for="(answer, i) in studentAnswers" :key="i">
            <h6>{{ answer.studentAnswer.name }}</h6>
            <article v-html="answer.studentAnswer.answer" />
          </b-list-group-item>
        </b-list-group>
      </b-collapse>
      <b-row no-gutters align-h="end" class="mt-3">
